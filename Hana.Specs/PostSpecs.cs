...
namespace Hana.Specs {
     ...
     * 
     * The Home is identified by a "slug", which is a semi-unique title made up of URL "safe"
     * characters that replace whitespace and non-URL friendly characters. The slug is created 
     * when the Home is, and can be specifed by the author or created automatically. The Slug is 
     ...
     * URL will further make it unique. If, for some dumb reason, I Home 2 posts on the same day
     ...
     * of sending out the excerpt thru RSS instead of whole Home. The excerpt will be defined by the user 
     * through LiveWriter, or determined by a method of the Home.
     * 
     * A Home can have one or more categories - with a default category of "blog". This is to fit in
     * with my left over Graffiti posts which used /category/slug and it's also good housekeeping
     * 
     * When a Home is created, a ping should be sent out to various services
     * 
     * A Home should have a status - intially offline or published
     * 
     * A Home should have a published date
     * 
     * A viewable Post = PublishedDate <= today and Status==published.
     * 
     * A new Home should require a title, author, body
     * 
     * A new Home should default published date to current date, accepting as override in ctor
     * 
    */
    ...
    public class creating_with_author_title_body : with_null_post {
        ...
        It should_have_proper_slug = () => {
            post.Slug.ShouldEqual("this-is-a-test-Home");
        };
        It should_have_Body = () => {
    ...
    public class object_overrides : with_null_post {
        ...
        It should_have_title_as_main_identifier = () => {
            post.ToString().ShouldEqual("This is a test Home");
        };
